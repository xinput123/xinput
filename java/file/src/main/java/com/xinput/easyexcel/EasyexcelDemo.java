...
import com.xinput.domain.RandomHelper;
...
public class EasyexcelDemo {
  public static void main(String[] args) {
    ...
    for (int i = 1; i <= 30; i++) {
      ...
      person.setId(i);
      person.setName(RandomHelper.getRandomName());
      person.setAge(RandomHelper.getRandomAge());
      person.setBirthday(day);
      person.setSex(RandomHelper.getRandomGender());
      persons.add(person);
    }
...
//        }
  ...
  @Test
  public void test001() {
    String fileName = "/Users/momo/Desktop/test001.xlsx";
    long start = System.currentTimeMillis();
    try (ExcelWriter excelWriter = EasyExcel.write(fileName, String.class).build()) {
      // 同一个sheet
      WriteSheet writeSheet = EasyExcel.writerSheet("sheet1").build();
      long lastId = 0L;
      int size = 200;
      while (true) {
        List<String> personList = getString(lastId, size);
        for (String i : personList) {
          lastId = Long.valueOf(i);
        }
        excelWriter.write(personList, writeSheet);
        if (personList.size() < size) {
          break;
  ...
  public List<Person> getData(long start, int size) {
    System.out.println("读取数据:" + start + ", 数量:" + size);
    String day = "2020-01-01";
  ...
  public List<String> getString(long start, int size) {
    System.out.println("读取数据:" + start + ", 数量:" + size);
    String day = "2020-01-01";
    List<String> persons = Lists.newArrayListWithExpectedSize(size);
    for (long i = start + 1; i <= start + size; i++) {
      if (i > 31000) {
        break;
      }
